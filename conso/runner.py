...
def task_bootstrap(_settings: Settings) -> RunOne:
    """First-time setup: verify session, auto-claim checkin, report X link."""
    async def _task(client: ConsoClient, state: AccountState) -> dict[str, Any]:
        ...
        state.bind_account(str(user.get("id") or ""))
        profile = await client.get_profile()
        if profile is None:
        ...
        out["x_linked"] = bool(profile.get("x_user_id"))
        out["x_username"] = profile.get("x_username")
        out["is_banned"] = bool(profile.get("is_banned"))
        # Auto-claim checkin.
        already = set(await client.get_todays_claims()) | set(state.claimed)
        claimed: list[str] = []
        skipped: list[str] = []
        for mission in await client.list_missions():
            mid = str(mission.get("id") or "")
            if not mid or mid in already:
                continue
            if str(mission.get("kind")) != "checkin":
                skipped.append(mid)
                continue
            ok, err = await client.claim_bonus_mission(mid)
            if ok:
                state.record_claim(mid)
                claimed.append(mid)
            else:
                skipped.append(f"{mid} (err: {err})")
        out["claimed_now"] = claimed
        out["skipped_missions"] = skipped
        if not out["x_linked"]:
            out["x_connect_hint"] = (
                "install the Conso extension and click 'Connect X' — "
                "the /api/extension/x flow requires a browser-signed "
                "state cookie only the extension can produce."
            )
        return out
    return _task
